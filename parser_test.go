...
func TestParserSearchForAnimeTitle(t *testing.T) {
	psr := getTestParser("[Anime Time] Neon Genesis Evangelion - Death.mkv")
	psr.searchForKeywords()
	psr.searchForIsolatedNumbers()
	psr.searchForEpisodeNumber()
	psr.searchForAnimeTitle()
	psr.postProcessing()
	if psr.tokenizer.elements.AnimeTitle != "Neon Genesis Evangelion - Death" {
		//t.Errorf("expected \"Toradora!\", got \"%s\"", psr.tokenizer.elements.AnimeTitle)
		t.Errorf("expected \"Neon Genesis Evangelion - Death\", got \"%s\"", psr.tokenizer.elements.AnimeTitle)
	}
}
